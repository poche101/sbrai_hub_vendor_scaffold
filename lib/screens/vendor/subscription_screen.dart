...
class _SubscriptionScreenState extends State<SubscriptionScreen> {
  ...
  Future<void> _pay() async {
    ...
    try {
      ...
      await _openCheckout(init.checkoutUrl, init.reference, verify: _subscriptionService.verifyPaystack);
    } catch (e) {
  ...
  Future<void> _payWithEspees() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text('Wallet ID', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
            const SizedBox(height: 6),
            TextField(controller: walletController, decoration: const InputDecoration(hintText: 'Your Espees wallet ID')),
            const SizedBox(height: 12),
            const Text('PIN', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
            const SizedBox(height: 6),
            TextField(
    ...
    if (walletController.text.trim().isEmpty || pinController.text.trim().length < 4) {
    ...
    try {
      ...
      if (vendorStatus.subscription.isActive) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Subscription active! You can now post ads.'), backgroundColor: AppColors.success),
        );
        context.pop();
      } else {
        setState(() => _error = 'Payment could not be confirmed. Please try again.');
      }
    } catch (_) {
      setState(() => _error = 'Espees payment failed. Please check your wallet ID and PIN and try again.');
    } finally {
      if (mounted) setState(() => _isProcessing = false);
    }
  }
  ...
  Future<void> _openCheckout(
    ...
    if (checkoutUrl == null) {
      setState(() => _error = 'Payment could not be initialized. Please try again shortly.');
      return;
    }
    ...
    if (PlatformSupport.supportsInAppWebView) {
      result = await Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => PaymentWebViewScreen(
            ...
            title: _method == _Method.paystack ? 'Pay with Paystack' : 'Pay with Espees',
          ),
        ),
      );
    } else {
      ...
      result = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: const Text('Once you\'ve finished paying, come back here and tap Confirm.'),
          actions: [
            TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')),
          ...
          builder: (context) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                TextField(controller: refController, decoration: const InputDecoration(hintText: 'e.g. T123456789')),
              ],
            ),
            actions: [
              TextButton(onPressed: () => Navigator.pop(context, null), child: const Text('Cancel')),
              ElevatedButton(
                onPressed: () => Navigator.pop(context, refController.text.trim()),
    ...
    try {
      ...
      if (vendorStatus.subscription.isActive) {
        ...
        setState(() => _error = 'Payment received but not yet confirmed. Please check back shortly.');
      }
    } catch (_) {
      setState(() => _error = 'Could not confirm your payment. If you were charged, please contact support.');
    }
  }
        ...
        child: Column(
          ...
          children: [
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 10),
                    const Text('Unlock Unlimited Ad Posting', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
                    const SizedBox(height: 6),
                    const Text(
            ...
            _MethodTile(
              ...
              subtitle: '₦${ApiConfig.subscriptionFeeNaira} — Card, bank transfer, or USSD',
